 ...
 */
export function createSkillStorePutTool(options) {
    ...
    return {
        ...
        async execute(params) {
            ...
            const textFields = [validated.title, validated.summary, validated.content].filter(Boolean);
            if (validated.data !== undefined) {
            ...
            if (validated.title)
                payload.title = sanitizeText(validated.title);
            if (validated.summary)
                payload.summary = sanitizeText(validated.summary);
            if (validated.content)
                payload.content = sanitizeText(validated.content);
            if (validated.data !== undefined)
                payload.data = validated.data;
            if (validated.media_url)
                payload.media_url = validated.media_url;
            if (validated.media_type)
                payload.media_type = validated.media_type;
            if (validated.source_url)
                payload.source_url = validated.source_url;
            if (validated.tags)
            ...
            if (validated.pinned !== undefined)
                payload.pinned = validated.pinned;
            if (validated.user_email)
                payload.user_email = validated.user_email;
            ...
            try {
                const response = await client.post('/skill-store/items', payload, { user_id });
                if (!response.success) {
 ...
 */
export function createSkillStoreGetTool(options) {
    const { client, logger, user_id } = options;
    return {
        ...
        async execute(params) {
            ...
            try {
                let response;
                if (validated.id) {
                    response = await client.get(`/skill-store/items/${validated.id}`, { user_id });
                }
                else {
                    // Guard above ensures skill_id and key are defined when id is absent
                    const skillId = validated.skill_id;
                    ...
                    response = await client.get(`/skill-store/items/by-key?${queryParams}`, { user_id });
                }
                if (!response.success) {
                ...
                if (item.content)
                    lines.push(`Content: ${truncateForPreview(item.content, 200)}`);
 ...
 */
export function createSkillStoreListTool(options) {
    const { client, logger, user_id } = options;
    return {
        ...
        async execute(params) {
            ...
            try {
                ...
                if (validated.collection)
                    queryParams.set('collection', validated.collection);
                if (validated.status)
                    queryParams.set('status', validated.status);
                if (validated.tags && validated.tags.length > 0)
                    queryParams.set('tags', validated.tags.join(','));
                if (validated.since)
                    queryParams.set('since', validated.since);
                if (validated.limit !== undefined)
                    queryParams.set('limit', String(validated.limit));
                if (validated.offset !== undefined)
                    queryParams.set('offset', String(validated.offset));
                if (validated.order_by)
                    queryParams.set('order_by', validated.order_by);
                if (validated.user_email)
                    queryParams.set('user_email', validated.user_email);
                const response = await client.get(`/skill-store/items?${queryParams}`, { user_id });
                if (!response.success) {
                    return {
                        success: false,
 ...
 */
export function createSkillStoreDeleteTool(options) {
    const { client, logger, user_id } = options;
    return {
        ...
        async execute(params) {
            ...
            try {
                ...
                const getResponse = await client.get(`/skill-store/items/by-key?${queryParams}`, { user_id });
...
export const SkillStoreSearchParamsSchema = z.object({
    skill_id: SkillIdSchema,
    query: z.string().min(1, 'Search query cannot be empty').max(2000, 'Search query must be 2000 characters or less'),
    collection: CollectionSchema.optional(),
    tags: z.array(z.string().max(100)).max(50).optional(),
...
function formatSearchResults(results) {
    ...
    return results
        ...
        const summary = r.summary ? `: ${truncateForPreview(r.summary)}` : r.content ? `: ${truncateForPreview(r.content)}` : '';
 ...
 */
function formatAggregateResult(operation, result) {
    switch (operation) {
        case 'count':
        ...
        case 'count_by_tag': {
            ...
            return (tags
                .filter((t) => t != null && typeof t === 'object' && 'tag' in t && 'count' in t)
                .map((t) => `- ${t.tag}: ${t.count}`)
                .join('\n') || 'No tags found.');
        }
        case 'count_by_status': {
            ...
            return (statuses
                .filter((s) => s != null && typeof s === 'object' && 'status' in s && 'count' in s)
 ...
 */
export function createSkillStoreSearchTool(options) {
    const { client, logger, user_id } = options;
    return {
        name: 'skill_store_search',
        description: 'Search skill store items by text or semantic similarity. Use when looking for stored data, ' +
        ...
        async execute(params) {
            ...
            try {
                ...
                if (validated.collection)
                    body.collection = validated.collection;
                if (validated.tags)
                    body.tags = validated.tags;
                if (validated.limit !== undefined)
                    body.limit = validated.limit;
                if (validated.user_email)
                    body.user_email = validated.user_email;
                if (useSemantic) {
                    if (validated.min_similarity !== undefined)
                        body.min_similarity = validated.min_similarity;
                    const response = await client.post('/skill-store/search/semantic', body, { user_id });
                    if (!response.success) {
                ...
                // Full-text search
                const response = await client.post('/skill-store/search', body, { user_id });
                if (!response.success) {
 ...
 */
export function createSkillStoreCollectionsTool(options) {
    const { client, logger, user_id } = options;
    return {
        ...
        async execute(params) {
            ...
            try {
                ...
                const response = await client.get(`/skill-store/collections?${queryParams}`, { user_id });
                if (!response.success) {
                ...
                const content = collections.map((c) => `- ${c.collection}: ${c.count} item${c.count !== 1 ? 's' : ''}`).join('\n');
 ...
 */
export function createSkillStoreAggregateTool(options) {
    const { client, logger, user_id } = options;
    return {
        name: 'skill_store_aggregate',
        description: 'Run simple aggregations on skill store items. Useful for understanding data volume, ' +
        ...
        async execute(params) {
            ...
            try {
                ...
                if (validated.since)
                    queryParams.set('since', validated.since);
                if (validated.until)
                    queryParams.set('until', validated.until);
                if (validated.user_email)
                    queryParams.set('user_email', validated.user_email);
                const response = await client.get(`/skill-store/aggregate?${queryParams}`, { user_id });
                if (!response.success) {
