 ...
 */
export function createTerminalSearchTool(options) {
    ...
    return {
        ...
        async execute(params) {
            ...
            try {
                const body = { query: sanitizedQuery, limit };
                if (connection_id)
                ...
                const response = await client.post('/terminal/search', body, { user_id });
                if (!response.success) {
                ...
                const content = entries
                    .map((e) => {
                    const parts = [`[${e.kind}]`];
                    if (e.session?.tmux_session_name)
                        parts.push(`session: ${e.session.tmux_session_name}`);
                    if (e.connection?.name)
                        parts.push(`on: ${e.connection.name}`);
                    if (e.similarity !== undefined)
                        parts.push(`(${(e.similarity * 100).toFixed(0)}% match)`);
 ...
 */
export function createTerminalAnnotateTool(options) {
    const { client, logger, user_id } = options;
    return {
        ...
        async execute(params) {
            ...
            try {
                const body = { content: sanitizedContent };
                if (tags)
                    body.tags = tags.split(',').map((t) => t.trim()).filter(Boolean);
                const response = await client.post(`/terminal/sessions/${session_id}/annotate`, body, { user_id });
                if (!response.success) {
