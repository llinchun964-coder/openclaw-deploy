...
const INTERNAL_ENTITY_TYPES = ['memory', 'todo', 'project', 'contact'];
/** All entity types including external references. */
const ALL_ENTITY_TYPES = [...INTERNAL_ENTITY_TYPES, 'github_issue', 'url'];
// ==================== Helpers ====================
/** UUID regex for validation */
const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
/** Check whether a target_type is internal (requires UUID). */
function isInternalType(type) {
    return INTERNAL_ENTITY_TYPES.includes(type);
}
/**
 ...
 */
function buildSourceTag(entity_type, entityRef) {
 ...
 */
export function createLinksSetTool(options) {
    const { client, logger, user_id } = options;
    return {
        name: 'links_set',
        description: 'Create a link between two entities (memory, todo, project, contact, GitHub issue, or URL). ' +
        ...
        async execute(params) {
            ...
            try {
                ...
                // Create forward link
                const forwardResponse = await client.post('/skill-store/items', {
                    skill_id: SKILL_ID,
                ...
                // Create reverse link
                const reverseResponse = await client.post('/skill-store/items', {
                    skill_id: SKILL_ID,
                    collection: COLLECTION,
                    key: reverseKey,
                    data: reverseData,
                ...
                if (!reverseResponse.success) {
                    ...
                    const rollback = await client.delete(`/skill-store/items/${forwardResponse.data.id}`, { user_id });
                    if (!rollback.success) {
                ...
                return {
                    ...
                    data: {
                        ...
                        details: {
                            ...
                            target_ref,
                            label: label ?? null,
                            forward_id: forwardResponse.data.id,
                            reverse_id: reverseResponse.data.id,
                            user_id,
                        },
                    },
                };
 ...
 */
export function createLinksQueryTool(options) {
    const { client, logger, user_id } = options;
    return {
        name: 'links_query',
        description: 'Query all links for an entity (memory, todo, project, or contact). Returns connected entities ' +
        ...
        async execute(params) {
            ...
            try {
                ...
                const response = await client.get(`/skill-store/items?${queryParams.toString()}`, { user_id });
                if (!response.success) {
                ...
                if (link_types && link_types.length > 0) {
                    ...
                    links = links.filter((link) => allowedTypes.has(link.target_type));
                }
                if (links.length === 0) {
                    return {
                        success: true,
 ...
 */
export function createLinksRemoveTool(options) {
    const { client, logger, user_id } = options;
    return {
        name: 'links_remove',
        description: 'Remove a link between two entities. Deletes both directions of the link. ' +
        ...
        async execute(params) {
            ...
            try {
                ...
                const forwardLookup = await client.get(`/skill-store/items/by-key?${forwardLookupParams.toString()}`, { user_id });
                ...
                const reverseLookup = await client.get(`/skill-store/items/by-key?${reverseLookupParams.toString()}`, { user_id });
                ...
                if (fwdNon404Error || revNon404Error) {
                    const errorMessages = [];
                    if (fwdNon404Error)
                        errorMessages.push(forwardLookup.error.message || 'Forward lookup failed');
                    if (revNon404Error)
                        errorMessages.push(reverseLookup.error.message || 'Reverse lookup failed');
                    return {
                        success: false,
                ...
                if (forwardLookup.success) {
                    expectedCount++;
                    const deleteResult = await client.delete(`/skill-store/items/${forwardLookup.data.id}`, { user_id });
                ...
                if (reverseLookup.success) {
                    expectedCount++;
                    const deleteResult = await client.delete(`/skill-store/items/${reverseLookup.data.id}`, { user_id });
                    if (deleteResult.success) {
                        deletedCount++;
                    }
                    else {
