...
function createCancellableTimeout(ms, timeoutResult) {
    let timerId;
...
export function createAutoRecallHook(options) {
    ...
    return async (event) => {
        ...
        const timeout = createCancellableTimeout(timeoutMs, null);
...
const RECALL_GUIDANCE_NOTE = '[Recalled from long-term memory. For deeper investigation, use memory_recall, context_search, or tool_guide.]';
/**
 ...
 */
async function fetchContext(client, user_id, prompt, logger, max_results = 5, minScore = 0.7, signal) {
    ...
    const response = await client.get(`/memories/search?${queryParams.toString()}`, { user_id, signal });
    if (!response.success) {
 ...
 */
function filterByScore(items, minScore, getScore) {
    if (items.length === 0)
        return [];
...
export function createAutoCaptureHook(options) {
    ...
    return async (event) => {
        ...
        const timeout = createCancellableTimeout(timeoutMs, 'timeout');
        try {
            // Race between API call and timeout
            const winner = await Promise.race([
                captureContext(client, user_id, event.messages, logger).then(() => 'done'),
 ...
 */
async function captureContext(client, user_id, messages, logger) {
    ...
    const response = await client.post('/context/capture', {
 ...
 */
async function fetchGraphAwareContext(client, user_id, user_email, prompt, logger, max_results = 10, minScore = 0.7, signal) {
    // Truncate prompt for the search query
    const searchPrompt = prompt.substring(0, 500);
    // Try graph-aware endpoint first
    const graphResponse = await client.post('/context/graph-aware', {
        prompt: searchPrompt,
