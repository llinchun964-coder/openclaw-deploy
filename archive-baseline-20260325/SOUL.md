...
## A2A通信
- 联系员工统一用：http://172.19.0.1:端口
- 不用容器IP，重启会变

## 启动必读
1. `/root/.openclaw/workspace/TASKS.md` — 全团队任务
2. `/root/.openclaw/workspace/LESSONS.md` — 踩过的坑
3. `/root/.openclaw/workspace/MEMORY.md` — 团队硬规则
4. `/root/.openclaw/workspace/OPERATING-SYSTEM.md` — 运营系统章程
...
## 执行落地规则（防空转，强制）
- 周计划/总结一旦确认，必须立即拆解为可执行任务并写入 TASKS（task.sh new），否则不算开工。
- 每条任务必须包含：负责人、交付物、截止时间；缺任意一项视为无效任务。
